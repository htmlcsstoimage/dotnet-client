...
public class QueryStringBuilderBenchmark
{
    ...
    public string BuiltInHttpUtility()
    {
    ...
    {
        var sb = new StringBuilder();
        sb.Append("url=").Append(Uri.EscapeDataString(_request.Url));
        sb.Append("&css=").Append(Uri.EscapeDataString(_request.Css!));
        sb.Append("&viewport_width=").Append(_request.ViewportWidth);
        sb.Append("&viewport_height=").Append(_request.ViewportHeight);
        sb.Append("&device_scale=").Append(_request.DeviceScale);
        sb.Append("&ms_delay=").Append(_request.MsDelay);
        sb.Append("&full_screen=true");
        sb.Append("&color_scheme=").Append(_request.ColorScheme?.ToString());
        sb.Append("&timezone=").Append(Uri.EscapeDataString(_request.Timezone!));
        return sb.ToString();
    }
    ...
    public string HctiQueryStringBuilder()
    {
        ...
        try
        {
            ...
            var result = new string(builder.QueryString(false));
            return result;
        }
        finally
        {
    ...
    public string HctiQueryStringBuilderDirect()
    {
        using QueryStringBuilder builder = new(stackalloc char[512]);
            ...
            builder.WriteSafeKey("viewport_width",_request.ViewportWidth!.Value);
            builder.WriteSafeKey("viewport_height",_request.ViewportHeight!.Value);
            builder.WriteSafeKey("device_scale",_request.DeviceScale!.Value);
            builder.WriteSafeKey("ms_delay",_request.MsDelay!.Value);
            builder.EncodeSafeKeyValue("full_screen","true");
            builder.EncodeSafeKeyValue("color_scheme",_request.ColorScheme!.Value.ColorSchemeString());
            builder.EncodeSafeKey("timezone",_request.Timezone);

            var result = builder.QueryString(false);
